...
#%% Create dataframe for oceanic
## CHL
oceanic_cluster = chl[clusters == 3,:]
oceanic_cluster = np.nanmean(oceanic_cluster,0)
oceanic_cluster_df_chl = pd.Series(data=oceanic_cluster, index=time_date_chl)
oceanic_cluster_df_monthly_chl = oceanic_cluster_df_chl.resample('M').mean()
oceanic_monthly_19972021_chl = oceanic_cluster_df_monthly_chl.values[:-8]
oceanic_monthly_19972021_index = oceanic_cluster_df_monthly_chl.index[:-8]
## SST
oceanic_cluster = sst[clusters == 3,:]
oceanic_cluster = np.nanmean(oceanic_cluster,0)
oceanic_cluster_df_sst = pd.Series(data=oceanic_cluster, index=time_date_sst)
oceanic_cluster_df_monthly_sst = oceanic_cluster_df_sst.resample('M').mean()
oceanic_monthly_19972021_sst = oceanic_cluster_df_monthly_sst.values[8:-8]
## Sea Ice
oceanic_cluster = seaice[clusters == 3,:]
oceanic_cluster = np.nanmean(oceanic_cluster,0)
oceanic_cluster_df_seaice = pd.Series(data=oceanic_cluster, index=time_date_seaice)
oceanic_cluster_df_monthly_seaice = oceanic_cluster_df_seaice.resample('M').mean()
oceanic_monthly_19972021_seaice = oceanic_cluster_df_monthly_seaice.values[8:-8]
## PAR
oceanic_cluster = par[clusters == 3,:]
oceanic_cluster = np.nanmean(oceanic_cluster,0)
oceanic_cluster_df_par = pd.Series(data=oceanic_cluster, index=time_date_par)
oceanic_cluster_df_monthly_par = oceanic_cluster_df_par.resample('M').mean()
oceanic_monthly_19972021_par = oceanic_cluster_df_monthly_par.values[:-17]
## Join all in the same dataframe
dataframe_oceanic = pd.DataFrame(data=oceanic_monthly_19972021_chl, index= oceanic_monthly_19972021_index, columns=['Chla'])
dataframe_oceanic['SST'] = oceanic_monthly_19972021_sst
#dataframe_oceanic['SEAICE'] = oceanic_monthly_19972021_seaice
dataframe_oceanic['PAR'] = oceanic_monthly_19972021_par
dataframe_oceanic['MEI'] = mei_monthly_19972021
dataframe_oceanic['SAM'] = sam_monthly_19972021
#%%
## Plot Correlation Matrix
corr = dataframe_oceanic.corr()
mask = np.triu(np.ones_like(corr, dtype=bool))
cmap = sns.diverging_palette(230, 20, as_cmap=True)
sns.heatmap(corr, annot=True, mask = mask, cmap=cmocean.cm.balance, vmin=-1, vmax=1)
#plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\randomforests\\oceanic\\chla_monthlyvariation\\')
graphs_dir = 'oceanic.png'
...
dataframe_all = dataframe_oceanic.dropna()
...
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\randomforests\\oceanic\\chla_monthlyvariation')
currentmodel_permut = pd.read_csv('permutation_oceanic_v1.csv', sep=";")
# Load sardine data
...
axs[1].scatter(dataframe_all.index, regressor.predict(X), marker='o', label = 'Predicted Chla', c='#d09c26', s=20, edgecolor='k')
...
axs[0].barh(variables_names[::-1], variables_importances[::-1], xerr=variables_stds[::-1], facecolor='#d09c26', edgecolor='k')
...
graphs_dir = 'oceanic_randomforestresults_V1withseaice.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
...
plt.tight_layout()
#os.chdir('C:\\Users\\afons\\Documents\\artigos\\sardine-2021\\analyses\\Random Forests\\NW')
graphs_dir = 'oceanic_randomforest_PAR.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
#%% Testes
plt.scatter(oceanic_monthly_19972021_chl, oceanic_monthly_19972021_sst)
np.corrcoef(oceanic_monthly_19972021_chl, oceanic_monthly_19972021_sst)
stats.linregress(oceanic_monthly_19972021_chl[~np.isnan(oceanic_monthly_19972021_chl)], oceanic_monthly_19972021_sst[~np.isnan(oceanic_monthly_19972021_chl)])
