...
#%% Create dataframe for gerlache
## CHL
gerlache_cluster = chl[clusters == 2,:]
gerlache_cluster = np.nanmean(gerlache_cluster,0)
gerlache_cluster_df_chl = pd.Series(data=gerlache_cluster, index=time_date_chl)
gerlache_cluster_df_monthly_chl = gerlache_cluster_df_chl.resample('M').mean()
gerlache_monthly_19972021_chl = gerlache_cluster_df_monthly_chl.values[:-8]
gerlache_monthly_19972021_index = gerlache_cluster_df_monthly_chl.index[:-8]
## SST
gerlache_cluster = sst[clusters == 2,:]
gerlache_cluster = np.nanmean(gerlache_cluster,0)
gerlache_cluster_df_sst = pd.Series(data=gerlache_cluster, index=time_date_sst)
gerlache_cluster_df_monthly_sst = gerlache_cluster_df_sst.resample('M').mean()
gerlache_monthly_19972021_sst = gerlache_cluster_df_monthly_sst.values[8:-8]
## Sea Ice
gerlache_cluster = seaice[clusters == 2,:]
gerlache_cluster = np.nanmean(gerlache_cluster,0)
gerlache_cluster_df_seaice = pd.Series(data=gerlache_cluster, index=time_date_seaice)
gerlache_cluster_df_monthly_seaice = gerlache_cluster_df_seaice.resample('M').mean()
gerlache_monthly_19972021_seaice = gerlache_cluster_df_monthly_seaice.values[8:-8]
## PAR
gerlache_cluster = par[clusters == 2,:]
gerlache_cluster = np.nanmean(gerlache_cluster,0)
gerlache_cluster_df_par = pd.Series(data=gerlache_cluster, index=time_date_par)
gerlache_cluster_df_monthly_par = gerlache_cluster_df_par.resample('M').mean()
gerlache_monthly_19972021_par = gerlache_cluster_df_monthly_par.values[:-17]
## Join all in the same dataframe
dataframe_gerlache = pd.DataFrame(data=gerlache_monthly_19972021_chl, index= gerlache_monthly_19972021_index, columns=['Chla'])
dataframe_gerlache['SST'] = gerlache_monthly_19972021_sst
#dataframe_gerlache['SEAICE'] = gerlache_monthly_19972021_seaice
dataframe_gerlache['PAR'] = gerlache_monthly_19972021_par
dataframe_gerlache['MEI'] = mei_monthly_19972021
dataframe_gerlache['SAM'] = sam_monthly_19972021
#%%
...
#plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\randomforests\\gerlache\\chla_monthlyvariation\\')
...
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\randomforests\\gerlache\\chla_monthlyvariation')
currentmodel_permut = pd.read_csv('permutation_gerlache_v1.csv', sep=";")
# Load sardine data
...
axs[1].scatter(dataframe_all.index, regressor.predict(X), marker='o', label = 'Predicted Chla', c=[215/256, 25/256, 28/256, 1], s=20, edgecolor='k')
...
axs[0].barh(variables_names[::-1], variables_importances[::-1], xerr=variables_stds[::-1], facecolor=[215/256, 25/256, 28/256, 1], edgecolor='k')
...
graphs_dir = 'gerlache_randomforestresults_V2withseaice.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
...
plot_partial_dependence(regressor,X, ['SST'], kind='average', 
...
plt.tight_layout()
#os.chdir('C:\\Users\\afons\\Documents\\artigos\\sardine-2021\\analyses\\Random Forests\\NW')
graphs_dir = 'gerlache_randomforest_PAR.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
#%% Testes
plt.scatter(gerlache_monthly_19972021_chl, gerlache_monthly_19972021_sst)
np.corrcoef(gerlache_monthly_19972021_chl, gerlache_monthly_19972021_sst)
stats.linregress(gerlache_monthly_19972021_chl[~np.isnan(gerlache_monthly_19972021_chl)], gerlache_monthly_19972021_sst[~np.isnan(gerlache_monthly_19972021_chl)])
