...
#%% re-set cycle to September-March
a = chl_averagecycle[201,300,:]
b = chl_averagecycle[203,300,:]
a = np.hstack((a[8:], a[:4]))
b = np.hstack((b[8:], b[:4]))
#a_more = np.vstack((a, b, a, b, a, b))
alignment = dtw.dtw(a, b, keep_internals=True)
#alignment = dtw.dtw(a_more, keep_internals=True)

#alignment.plot(type="threeway")
alignment.plot(type="twoway") 
dist = alignment.distance
#dtw.dtwPlot(alignment, type="twoway")
#%%
# Initialize distance matrix
n_series = len(lat)*len(lon)
distance_matrix = np.zeros(shape=(n_series, n_series), dtype=np.float16)
# Convert chl 2D data to 1D
chl_averacycle_1D = np.reshape(chl_averagecycle, (n_series, 12))
# c
# Build distance matrix       
for k in tqdm(range(n_series)):
    for f in range(n_series):
        x = chl_averacycle_1D[k]
        x = np.hstack((x[8:], x[:4]))
        y = chl_averacycle_1D[f]
        y = np.hstack((y[8:], y[:4]))
        if np.isnan(x).all() or np.isnan(y).all():
            distance_matrix[k, f] = np.nan
            continue
        if k != f:
            dist = dtw.dtw(x, y, distance_only=True)
            distance_matrix[k, f] = dist.distance




#%%
distmatrix = np.empty((529*1153))
for i in range(0, len(lat)):
    for j in range(0, len(lon)):
        x_pixel_cycle = chl_averagecycle[i,j,:]
        x_pixel_cycle_aligned = np.hstack((x_pixel_cycle[8:], x_pixel_cycle[:4]))
        dist_temp = []
        for k in tqdm(range(0, len(lat))):
            for l in range(0, len(lon)):
                y_pixel_cycle = chl_averagecycle[k,l,:]
                y_pixel_cycle_aligned = np.hstack((y_pixel_cycle[8:], y_pixel_cycle[:4]))
                if np.isnan(y_pixel_cycle_aligned).any():
                    dist_temp = np.append(dist_temp, np.nan)
                else:
                    # calculate distance
                    alignment = dtw.dtw(x_pixel_cycle_aligned, y_pixel_cycle_aligned, keep_internals=False, distance_only=True)
                    dist_temp = np.append(dist_temp, alignment.distance)
                
                # add to matrix
f = np.reshape(dist_temp, (-1, 1153))             
#%%
...
f1 = map.pcolormesh(lon, lat, f[:-1, :-1], transform=ccrs.PlateCarree(), shading='flat', cmap=plt.cm.twilight_shifted,
                    vmin=1, vmax=12)
...
plt.tight_layout()
graphs_dir = 'C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\chl\\phenology\\maxchlday_monthlydata.png'
...
#cbar.set_label('Valid Pixels (%)', fontsize=14)
plt.tight_layout()
graphs_dir = 'C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\chl\\phenology\\startchlday_monthlydata.png'
plt.savefig(graphs_dir,format = 'png', bbox_inches = 'tight', dpi = 300)
plt.close()
## Final Chl
plt.figure()
...
#cbar.set_label('Valid Pixels (%)', fontsize=14)
plt.tight_layout()
graphs_dir = 'C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\chl\\phenology\\endchlday_monthlydata.png'
plt.savefig(graphs_dir,format = 'png', bbox_inches = 'tight', dpi = 300)
plt.close()
