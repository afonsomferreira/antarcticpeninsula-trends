...
chl_decfeb19982021 = np.nanmean(chl[:,:, (time_date_months >= 12) | (time_date_months <= 2)],2)
...
chl_decfeb_1D = chl_decfeb19982021.ravel()
...
chl_decfeb_1D_nonan = chl_decfeb_1D[~np.isnan(chl_decfeb_1D) & ~np.isnan(chl_max_1D)]
...
df = pd.DataFrame(x)
df = df.T
df.columns = ['Peak', 'Max', 'Initiation', 'Termination', 'Sep-Apr Mean', 'Dec-Feb Mean']# 'PAR First', 'PAR Last']
import seaborn as sns
corr = df.corr()
...
features = ['Peak', 'Initiation', 'Termination']
x = df.loc[:, features]
#x = x.dropna().values
