...
for i in range(0,len(lat_seaice)):
    ...
    for j in range(0,len(lon_seaice)):
        ...
        if np.nanmin(seaice_pixel_df_sepapr) >=30:
            seaice_50mask[i,j] = 1
        else:
            seaice_50mask[i,j] = 0
#%%
...
plt.tight_layout()
graphs_dir = 'C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\clusters_sepapr_seaice_min30per.png'
...
#                                        facecolor=cartopy.feature.COLORS['land']))
#gl = map.gridlines(draw_labels=True, alpha=0.5, linestyle='dotted', color='black')
#poly_CBS = Polygon(list(oceanic_winds_verts), facecolor=[1,1,1,0], edgecolor='k', linewidth=1, linestyle='--', zorder=2, transform=ccrs.PlateCarree())
...
p = Path(oceanic_winds_verts) # make a polygon
...
winds_northward_oceanic = np.ma.array(winds_northward, mask=~mask3d)
winds_northward_oceanic = np.nanmean(winds_northward_oceanic, (0,1))
## Eastward wind
x, y = np.meshgrid(lon_winds, lat_winds) # make a canvas with coordinates
x, y = x.flatten(), y.flatten()
points = np.vstack((x, y)).T
...
mask3d = np.swapaxes(mask3d, 0, 1)
mask3d = np.swapaxes(mask3d, 1, 2)
winds_eastward_oceanic = np.ma.array(winds_eastward, mask=~mask3d)
winds_eastward_oceanic = np.nanmean(winds_eastward_oceanic, (0,1))
## Calculate wind speed
wind_speed = np.sqrt(winds_eastward_oceanic**2 + winds_northward_oceanic**2)
wind_dir = np.empty_like(wind_speed)
for k in range(0, len(wind_dir)):
    wind_dir_trig_to = math.atan2(winds_eastward_oceanic[k]/wind_speed[k], winds_northward_oceanic[k]/wind_speed[k]) 
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\clusters_withoutseaice_min30\\')
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\clusters_withoutseaice_min30\\')
graphs_dir = 'oceanic_sepapr_phenology.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\clusters_withoutseaice_min30\\')
graphs_dir = 'bransfield_sepapr_climatology_comparison5years_windpseed.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
# Comparing just prior and post 2005
fig, axs = plt.subplots(1, 1, figsize=(11,4))
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\clusters_withoutseaice_min30\\')
graphs_dir = 'bransfield_sepapr_climatology_comparison_prepost2005_windspeed.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()

#%% Setember-Dec Mean from 1981 to 2021
# CHL
chl_bransfield = chl[clusters == 3,:]
...
# SST
sst_bransfield_19972021 = sst[clusters == 3,:]
sst_bransfield_19972021 = np.nanmean(sst_bransfield_19972021,0)
sst_bransfield_19811996 = sst_19811996[clusters == 3,:]
