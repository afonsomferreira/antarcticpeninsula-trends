...
oceanic_verts_1 = [(-66, -60.5),
             (-63, -60.5),
             (-63, -62),
             (-66, -62)]
oceanic_verts_2 = [(-53.5, -62.7),
             (-52.5, -62.7),
             (-52.5, -65),
             (-53.5, -65)]
#%% Create dataframe for oceanic using spring-summer data (September-April means)
leapyears_list = [1980, 1984, 1988, 1992, 1996, 2000, 2004, 2008, 2012, 2016, 2020, 2024]
x, y = np.meshgrid(lon_chl, lat_chl) # make a canvas with coordinates
x, y = x.flatten(), y.flatten()
points = np.vstack((x, y)).T
p = Path(oceanic_verts_1) # make a polygon
grid = p.contains_points(points)
mask = grid.reshape(len(lon_chl), len(lat_chl))
mask3d = np.repeat([mask], np.size(chl,2), axis=0)
mask3d = np.swapaxes(mask3d, 0, 1)
mask3d = np.swapaxes(mask3d, 1, 2)
chl_oceanic = np.ma.array(chl, mask=~mask3d)
chl_oceanic = np.nanmean(chl_oceanic, (0,1))
## CHL #1998-2021
...
## SST 1997-2021
x, y = np.meshgrid(lon_sst, lat_sst) # make a canvas with coordinates
x, y = x.flatten(), y.flatten()
points = np.vstack((x, y)).T
p = Path(oceanic_verts_1) # make a polygon
grid = p.contains_points(points)
mask = grid.reshape(len(lon_sst), len(lat_sst))
mask3d = np.repeat([mask], np.size(sst,2), axis=0)
mask3d = np.swapaxes(mask3d, 0, 1)
mask3d = np.swapaxes(mask3d, 1, 2)
sst_oceanic_19972021 = np.ma.array(sst, mask=~mask3d)
sst_oceanic_19972021 = np.nanmean(sst_oceanic_19972021, (0,1))
## SST 1981-1996
x, y = np.meshgrid(lon_sst, lat_sst) # make a canvas with coordinates
x, y = x.flatten(), y.flatten()
points = np.vstack((x, y)).T
...
grid = p.contains_points(points)
mask = grid.reshape(len(lon_sst), len(lat_sst))
mask3d = np.repeat([mask], np.size(sst_19811996,2), axis=0)
mask3d = np.swapaxes(mask3d, 0, 1)
mask3d = np.swapaxes(mask3d, 1, 2)
sst_oceanic_19811996 = np.ma.array(sst_19811996, mask=~mask3d)
sst_oceanic_19811996 = np.nanmean(sst_oceanic_19811996, (0,1))
## Add both
...
## Sea ice
x, y = np.meshgrid(lon_seaice, lat_seaice) # make a canvas with coordinates
x, y = x.flatten(), y.flatten()
points = np.vstack((x, y)).T
p = Path(oceanic_verts_1) # make a polygon
grid = p.contains_points(points)
mask = grid.reshape(len(lon_seaice), len(lat_seaice))
mask3d = np.repeat([mask], np.size(seaice,2), axis=0)
mask3d = np.swapaxes(mask3d, 0, 1)
mask3d = np.swapaxes(mask3d, 1, 2)
seaice_oceanic_19972021 = np.ma.array(seaice, mask=~mask3d)
seaice_oceanic_19972021 = np.nanmean(seaice_oceanic_19972021, (0,1))
## SST 1981-1996
x, y = np.meshgrid(lon_seaice, lat_seaice) # make a canvas with coordinates
x, y = x.flatten(), y.flatten()
points = np.vstack((x, y)).T
...
grid = p.contains_points(points)
mask = grid.reshape(len(lon_seaice), len(lat_seaice))
mask3d = np.repeat([mask], np.size(seaice_19811996,2), axis=0)
mask3d = np.swapaxes(mask3d, 0, 1)
mask3d = np.swapaxes(mask3d, 1, 2)
seaice_oceanic_19811996 = np.ma.array(seaice_19811996, mask=~mask3d)
seaice_oceanic_19811996 = np.nanmean(seaice_oceanic_19811996, (0,1))
## Add both
...
## PAR
x, y = np.meshgrid(lon_par, lat_par) # make a canvas with coordinates
x, y = x.flatten(), y.flatten()
points = np.vstack((x, y)).T
p = Path(oceanic_verts_1) # make a polygon
grid = p.contains_points(points)
mask = grid.reshape(len(lon_par), len(lat_par))
mask3d = np.repeat([mask], np.size(par,2), axis=0)
mask3d = np.swapaxes(mask3d, 0, 1)
mask3d = np.swapaxes(mask3d, 1, 2)
par_oceanic = np.ma.array(par, mask=~mask3d)
par_oceanic = np.nanmean(par_oceanic, (0,1))
...
axs.set_xticks(ticks = [1, 4, 8, 12, 16, 20, 23, 27, 31, 35, 38, 42],
           labels = ['Aug', 'Sep', 'Oct', 'Nov', 'Dec', 'Jan', 'Feb', 'Mar', 'Apr', 'May',
                     'Jun', 'Jul'], fontsize=10)
plt.axvline(4, linestyle='--', c='grey', alpha=0.3)
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_NAREA_sepapr_climatology.png'
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_NAREA_sepapr_climatology_comparison5years.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_NAREA_sepapr_climatology_comparison_prepost2005.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
#%% Separate and calculate changes to yearly cycles
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_NAREA_sepapr_climatology_comparison5years_par.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
# Comparing just prior and post 2005
fig, axs = plt.subplots(1, 1, figsize=(11,4))
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_NAREA_sepapr_climatology_comparison_prepost2005_par.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
#%% Separate and calculate changes to yearly cycles
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_NAREA_sepapr_climatology_comparison10years_sst.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
# Comparing just prior and post 2005
fig, axs = plt.subplots(1, 1, figsize=(11,4))
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_NAREA_sepapr_climatology_comparison_prepost2005_sst.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
#%% Separate and calculate changes to yearly cycles
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_NAREA_sepapr_climatology_comparison10years_seaice.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
# Comparing just prior and post 2005
fig, axs = plt.subplots(1, 1, figsize=(11,4))
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_NAREA_sepapr_climatology_comparison_prepost2005_seaice.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
#%%
#%% Create dataframe for oceanic using spring-summer data (September-April means)
...
x, y = x.flatten(), y.flatten()
points = np.vstack((x, y)).T
p = Path(oceanic_verts_2) # make a polygon
grid = p.contains_points(points)
mask = grid.reshape(len(lon_chl), len(lat_chl))
mask3d = np.repeat([mask], np.size(chl,2), axis=0)
mask3d = np.swapaxes(mask3d, 0, 1)
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_SAREA_sepapr_climatology.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
#%% Separate and calculate changes to yearly cycles
chl_sepapr_19982003 = np.nanmean(chl_sepapr_19982021[:6, :],0)
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_SAREA_sepapr_climatology_comparison5years.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
# Comparing just prior and post 2005
fig, axs = plt.subplots(1, 1, figsize=(11,4))
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_SAREA_sepapr_climatology_comparison_prepost2005.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
#%% Separate and calculate changes to yearly cycles
par_sepapr_19982003 = np.nanmean(par_sepapr_19982021[:6, :],0)
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_SAREA_sepapr_climatology_comparison5years_par.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
# Comparing just prior and post 2005
fig, axs = plt.subplots(1, 1, figsize=(11,4))
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_SAREA_sepapr_climatology_comparison_prepost2005_par.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
#%% Separate and calculate changes to yearly cycles
sst_sepapr_19821991 = np.nanmean(sst_sepapr_19982021[:10, :],0)
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_SAREA_sepapr_climatology_comparison10years_sst.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
# Comparing just prior and post 2005
fig, axs = plt.subplots(1, 1, figsize=(11,4))
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_SAREA_sepapr_climatology_comparison_prepost2005_sst.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
#%% Separate and calculate changes to yearly cycles
seaice_sepapr_19821991 = np.nanmean(seaice_sepapr_19982021[:10, :],0)
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_SAREA_sepapr_climatology_comparison10years_seaice.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
# Comparing just prior and post 2005
fig, axs = plt.subplots(1, 1, figsize=(11,4))
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\comparingoceanicregions\\')
graphs_dir = 'oceanic_SAREA_sepapr_climatology_comparison_prepost2005_seaice.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
