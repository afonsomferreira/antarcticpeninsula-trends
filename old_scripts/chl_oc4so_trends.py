...
map.pcolor(lon, lat, chl_summertrend19992020_significant[:-1, :-1], hatch='...', alpha=0., transform=ccrs.PlateCarree(), shading='flat')
gl = map.gridlines(draw_labels=True, alpha=0.5, linestyle='dotted', color='black')
cbar = plt.colorbar(f1,
                    fraction=0.04, pad=0.1)
#cbar.ax.set_yticklabels(['0.1', '0.5', '1', '3', '10'], fontsize=14)
cbar.set_label('Change in Nov-Feb Chl-$\it{a}$ (mg.m$^{-3}$)', fontsize=16)
#map.contour(lon, lat, chl_summertrend19992020_significant, levels = [0.9, 1.1], colors='k',
#            transform=ccrs.PlateCarree())
#plt.contour(X, Y, Z, levels=[0.5, 0.75], colors=['black','cyan'])
#cbar.set_label('Valid Pixels (%)', fontsize=14)
...
map.add_feature(cartopy.feature.NaturalEarthFeature('physical', 'land', '50m',
                                        ...
                                        facecolor=cartopy.feature.COLORS['land']))
plt.tight_layout()
graphs_dir = 'C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\chl\\trends\\summertrend_4km.png'
plt.savefig(graphs_dir,format = 'png', bbox_inches = 'tight', dpi = 300)
plt.close()
...
map.add_feature(cartopy.feature.NaturalEarthFeature('physical', 'land', '50m',
                                        ...
                                        facecolor=cartopy.feature.COLORS['land']))
plt.tight_layout()
graphs_dir = 'C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\chl\\trends\\summertrendsignificant_4km.png'
plt.savefig(graphs_dir,format = 'png', bbox_inches = 'tight', dpi = 300)
plt.close()
#%%
xx, yy = np.meshgrid(lon, lat)
