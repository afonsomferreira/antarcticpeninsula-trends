...
#%% Create dataframe for weddell using spring-summer data (September-April means)
...
#poly_CBS = Polygon(list(weddell_winds_verts), facecolor=[1,1,1,0], edgecolor='k', linewidth=1, linestyle='--', zorder=2, transform=ccrs.PlateCarree())
...
p = Path(weddell_winds_verts) # make a polygon
...
winds_northward_weddell = np.ma.array(winds_northward, mask=~mask3d)
winds_northward_weddell = np.nanmean(winds_northward_weddell, (0,1))
## Eastward wind
x, y = np.meshgrid(lon_winds, lat_winds) # make a canvas with coordinates
x, y = x.flatten(), y.flatten()
points = np.vstack((x, y)).T
...
mask3d = np.swapaxes(mask3d, 0, 1)
mask3d = np.swapaxes(mask3d, 1, 2)
winds_eastward_weddell = np.ma.array(winds_eastward, mask=~mask3d)
winds_eastward_weddell = np.nanmean(winds_eastward_weddell, (0,1))
## Calculate wind speed
wind_speed = np.sqrt(winds_eastward_weddell**2 + winds_northward_weddell**2)
wind_dir = np.empty_like(wind_speed)
for k in range(0, len(wind_dir)):
    wind_dir_trig_to = math.atan2(winds_eastward_weddell[k]/wind_speed[k], winds_northward_weddell[k]/wind_speed[k]) 
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\figures_tentative\\')
graphs_dir = 'weddell_sepapr_phenology.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
...
# SST
sst_bransfield_19972021 = sst[clusters == 1,:]
sst_bransfield_19972021 = np.nanmean(sst_bransfield_19972021,0)
sst_bransfield_19811996 = sst_19811996[clusters == 1,:]
