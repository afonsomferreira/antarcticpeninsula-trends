...
yearlyaveragecycles = np.empty((529, 1153, 52))
for i in tqdm(range(0, len(lat))):
    for j in range(0, len(lon)):
        ...
        pixel_alldata_series_weekly = pixel_alldata_series.resample('W-MON', label='left',closed='left').mean()
        if pixel_alldata_series.isnull().all():
            yearlyaveragecycles[i, j, :] = np.empty((52))*np.nan
            continue
        else:
            #pixel_alldata_series = pixel_alldata_series[~((pixel_alldata_series.index.month == 2) & (pixel_alldata_series.index.day == 29))]
            pixel_yearlyaveragecycle = pixel_alldata_series.groupby([pixel_alldata_series.index.isocalendar().week]).mean().values
            yearlyaveragecycles[i, j, :] = pixel_yearlyaveragecycle[:-1]
# Save datasets with lower resolution
np.savez_compressed('chloc4so_19972021_yearlyaveragecycles_weekly', chl_averagecycle=yearlyaveragecycles,time_date=time_date,
                    lat=lat, lon=lon)
