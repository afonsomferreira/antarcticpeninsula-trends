...
### Load data 1998-2020
fh = np.load('chloc4so_19972021_yearlyaveragecycles.npz', allow_pickle=True)
lat = fh['lat']
lon = fh['lon']
...
for i in range(0, len(lat)):
    for j in range(0, len(lon)):
        if np.count_nonzero(np.isnan(chl_averagecycle[i,j,:])) > 300:
            continue
        # find month where chl reaches its max
        startDate = datetime.datetime(year=1999, month=1, day=1)
        day_chl_max = np.nanargmax(chl_averagecycle[i,j,:]) + 1
        daysToShift = day_chl_max - 1
        endDate = startDate + datetime.timedelta(days=int(daysToShift))
        ...
        try:
            dayfirstchlafterwinter = next(i for i,v in enumerate(chl_averagecycle[i,j,:][170:]) if v > 0) + 171
        except:
            dayfirstchlafterwinter = next(i for i,v in enumerate(chl_averagecycle[i,j,:]) if v > 0)
        endDate = startDate + datetime.timedelta(days=int(dayfirstchlafterwinter))
        ...
        daylastchl = 171 - next(i for i,v in enumerate(np.flip(chl_averagecycle[i,j,:])[194:]) if v > 0)
        endDate = startDate + datetime.timedelta(days=int(daylastchl))
...
plt.tight_layout()
graphs_dir = 'C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\chl\\phenology\\maxchlday_dailydata.png'
...
#cbar.set_label('Valid Pixels (%)', fontsize=14)
plt.tight_layout()
graphs_dir = 'C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\chl\\phenology\\startchlday_dailydata.png'
plt.savefig(graphs_dir,format = 'png', bbox_inches = 'tight', dpi = 300)
plt.close()
## Final Chl
plt.figure()
...
#cbar.set_label('Valid Pixels (%)', fontsize=14)
plt.tight_layout()
graphs_dir = 'C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\chl\\phenology\\endchlday_dailydata.png'
plt.savefig(graphs_dir,format = 'png', bbox_inches = 'tight', dpi = 300)
plt.close()
