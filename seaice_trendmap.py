...
for i in np.arange(1998, 2022):
    yeartemp_sep = seaice[:,:, (time_date_years == i-1) & (time_date_months == 9)]
    yeartemp_oct = seaice[:,:, (time_date_years == i-1) & (time_date_months == 10)]
    yeartemp_nov = seaice[:,:, (time_date_years == i-1) & (time_date_months == 11)]
    yeartemp_dec = seaice[:,:, (time_date_years == i-1) & (time_date_months == 12)]
    yeartemp_jan = seaice[:,:, (time_date_years == i) & (time_date_months == 1)]
    yeartemp_feb = seaice[:,:, (time_date_years == i) & (time_date_months == 2)]
    yeartemp_mar = seaice[:,:, (time_date_years == i) & (time_date_months == 3)]
    yeartemp_apr = seaice[:,:, (time_date_years == i) & (time_date_months == 4)]
...
for i in range(0, len(lat)):
    ...
    for j in range(0, len(lon)):
        yearchar = np.arange(1998, 2022)
        seaice_summerpixel = years_summermeans[i,j,:]
...
xx = xx[~np.isnan(seaice_summertrend19992020_significant)]
yy = yy[~np.isnan(seaice_summertrend19992020_significant)]
plt.figure()
...
map.contourf(
    lon, lat, seaice_summertrend19992020_significant,
    transform=ccrs.PlateCarree(),
    colors='none',
    hatches=[3*'.',3*'.'],
)
#map.contour(lon, lat, seaice_summertrend19992020_significant, levels=[0, 0.01], colors='black', zorder=1, transform=ccrs.PlateCarree())
...
cbar.set_label('Sea Ice (%)', fontsize=14)
...
plt.tight_layout()
graphs_dir = 'C:\\Users\\afons\\OneDrive - Universidade de Lisboa\\Documents\\artigos\\antarctic-peninsula-trends-2021\\analysis\\seaice_trendmap_SEPAPR.png'
...
map = plt.axes(projection=ccrs.AzimuthalEquidistant(central_longitude=-60, central_latitude=-62))
map.set_extent([-67, -53, -67, -60])
f1 = map.pcolormesh(lon, lat, seaice_summertrend19992020_significant[:-1, :-1], transform=ccrs.PlateCarree(), shading='flat', vmin=-1, vmax=1,
                    cmap=cmocean.cm.balance, zorder=0)
gl = map.gridlines(draw_labels=True, alpha=0.5, linestyle='dotted', color='black')
cbar = plt.colorbar(f1, fraction=0.04, pad=0.1)
#cbar.ax.set_yticklabels(['0.1', '0.5', '1', '3', '10'], fontsize=14)
cbar.set_label('Sea Ice (%)', fontsize=14)
#map.contour(lon, lat, seaice_summertrend19992020_significant, levels = [0.9, 1.1], colors='k',
#            transform=ccrs.PlateCarree())
#plt.contour(X, Y, Z, levels=[0.5, 0.75], colors=['black','cyan'])
#cbar.set_label('Valid Pixels (%)', fontsize=14)
map.coastlines(resolution='10m', color='black', linewidth=1)
map.add_feature(cartopy.feature.NaturalEarthFeature('physical', 'land', '50m',
                                        ...
                                        facecolor=cartopy.feature.COLORS['land']))
plt.tight_layout()
graphs_dir = 'C:\\Users\\afons\\OneDrive - Universidade de Lisboa\\Documents\\artigos\\antarctic-peninsula-trends-2021\\analysis\\seaice_trendmap_SEPAPR_onlysignificant.png'
plt.savefig(graphs_dir,format = 'png', bbox_inches = 'tight', dpi = 300)
plt.close()