...
### Load data 1998-2020
fh = np.load('chloc4so_19972021_20km.npz', allow_pickle=True)
lat = fh['lat']
lon = fh['lon']
chl = fh['chl']
...
yearlyaveragecycles = np.empty((110, 240, 12))
for i in range(0, len(lat)):
    for j in range(0, len(lon)):
        ...
        if pixel_alldata_series.isnull().all():
            yearlyaveragecycles[i, j, :] = np.empty((12))*np.nan
            continue
        else:
            #pixel_alldata_series = pixel_alldata_series[~((pixel_alldata_series.index.month == 2) & (pixel_alldata_series.index.day == 29))]
            pixel_yearlyaveragecycle = pixel_alldata_series.groupby([pixel_alldata_series.index.month]).mean().values
            yearlyaveragecycles[i, j, :] = pixel_yearlyaveragecycle
# Save datasets with lower resolution
np.savez_compressed('chloc4so_19972021_yearlyaveragecycles_month_20km', chl_averagecycle=yearlyaveragecycles,time_date=time_date,
                    lat=lat, lon=lon)
