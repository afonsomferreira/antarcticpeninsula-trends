...
for i in np.arange(1998, 2022):
    ...
    if i == 1998:
        years_sst_summermeans = yeartemp_summermean
    else:
        years_sst_summermeans = np.dstack((years_sst_summermeans, yeartemp_summermean))
...
for i in range(0, len(lat)):
    for j in range(0, len(lon)):
        yearchar = np.arange(1998, 2022)
        sst_summerpixel = years_sst_summermeans[i,j,:]
...
map.pcolor(lon, lat, sst_summertrend19992020_significant[:-1, :-1], hatch='...', alpha=0., transform=ccrs.PlateCarree(), shading='flat')
...
cbar.set_label('Change in Nov-Feb SST (degC)', fontsize=16)
...
plt.tight_layout()
graphs_dir = 'C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\sst\\trends\\summertrend_4km.png'
...
for i in np.arange(1998, 2022):
    ...
    yeartemp_summermean = np.nanmean(np.dstack((yeartemp_nov, yeartemp_dec, yeartemp_jan, yeartemp_feb)),2)
    if i == 1998:
        years_seaice_summermeans = yeartemp_summermean
    else:
        years_seaice_summermeans = np.dstack((years_seaice_summermeans, yeartemp_summermean))
#%% Calculate linear trend for each pixel
yearchar = np.arange(1998, 2022)
