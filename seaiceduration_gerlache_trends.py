...
### Load data 1998-2020
fh = np.load('sst_19812021.npz', allow_pickle=True)
lat = fh['lat']
lon = fh['lon']
seaice = fh['seaice']
seaice = seaice*100
...
# Load clusters
#%%
south_verts=[(-63.7, -65.1), (-63.7,-64.85), (-62.2,-64.35), (-61.8,-64), (-61.1, -64.1), (-60.8, -64.2)]
### Calculate Average cycle for NW
x, y = np.meshgrid(lon, lat) # make a canvas with coordinates
x, y = x.flatten(), y.flatten()
points = np.vstack((x, y)).T
p = Path(south_verts) # make a polygon
grid = p.contains_points(points)
mask = grid.reshape(len(lon), len(lat))
mask3d = np.repeat([mask], np.size(seaice,2), axis=0)
mask3d = np.swapaxes(mask3d, 0, 1)
mask3d = np.swapaxes(mask3d, 1, 2)
seaice_gerlache = np.ma.array(seaice, mask=~mask3d)
seaice_gerlache = np.nanmean(seaice_gerlache, (0,1))
#%%
for i in np.arange(1982, 2022):
    yeartemp_sep = seaice_gerlache[(time_date_years == i-1) & (time_date_months == 9)]
    yeartemp_oct = seaice_gerlache[(time_date_years == i-1) & (time_date_months == 10)]
    yeartemp_nov = seaice_gerlache[(time_date_years == i-1) & (time_date_months == 11)]
    yeartemp_dec = seaice_gerlache[(time_date_years == i-1) & (time_date_months == 12)]
    yeartemp_jan = seaice_gerlache[(time_date_years == i) & (time_date_months == 1)]
    yeartemp_feb = seaice_gerlache[(time_date_years == i) & (time_date_months == 2)]
    yeartemp_mar = seaice_gerlache[(time_date_years == i) & (time_date_months == 3)]
    yeartemp_apr = seaice_gerlache[(time_date_years == i) & (time_date_months == 4)]
...
# September
slope_weddell_sep, _, rvalue_weddell_sep, pvalue_weddell_sep, _ = stats.linregress(np.arange(1982, 2022)[~np.isnan(years_sep_19982021)], years_sep_19982021[~np.isnan(years_sep_19982021)])
# October
slope_weddell_oct, _, rvalue_weddell_oct, pvalue_weddell_oct, _ = stats.linregress(np.arange(1982, 2022)[~np.isnan(years_oct_19982021)], years_oct_19982021[~np.isnan(years_oct_19982021)])
# November
slope_weddell_nov, _, rvalue_weddell_nov, pvalue_weddell_nov, _ = stats.linregress(np.arange(1982, 2022)[~np.isnan(years_nov_19982021)], years_nov_19982021[~np.isnan(years_nov_19982021)])
# December
slope_weddell_dec, _, rvalue_weddell_dec, pvalue_weddell_dec, _ = stats.linregress(np.arange(1982, 2022)[~np.isnan(years_dec_19982021)], years_dec_19982021[~np.isnan(years_dec_19982021)])
# January
slope_weddell_jan, _, rvalue_weddell_jan, pvalue_weddell_jan, _ = stats.linregress(np.arange(1982, 2022)[~np.isnan(years_jan_19982021)], years_jan_19982021[~np.isnan(years_jan_19982021)])
# February
slope_weddell_feb, _, rvalue_weddell_feb, pvalue_weddell_feb, _ = stats.linregress(np.arange(1982, 2022)[~np.isnan(years_feb_19982021)], years_feb_19982021[~np.isnan(years_feb_19982021)])
# March
slope_weddell_mar, _, rvalue_weddell_mar, pvalue_weddell_mar, _ = stats.linregress(np.arange(1982, 2022)[~np.isnan(years_mar_19982021)], years_mar_19982021[~np.isnan(years_mar_19982021)])
# April # More than 5 years with nans
slope_weddell_apr, _, rvalue_weddell_apr, pvalue_weddell_apr, _ = stats.linregress(np.arange(1982, 2022)[~np.isnan(years_apr_19982021)], years_apr_19982021[~np.isnan(years_apr_19982021)])
#slope_weddell_apr = np.nan
# November-February
slope_weddell_novfeb, _, rvalue_weddell_novfeb, pvalue_weddell_novfeb, _ = stats.linregress(np.arange(1982, 2022)[~np.isnan(years_novfeb_19982021)], years_novfeb_19982021[~np.isnan(years_novfeb_19982021)])
# September-April
slope_weddell_sepapr, _, rvalue_weddell_sepapr, pvalue_weddell_sepapr, _ = stats.linregress(np.arange(1982, 2022)[~np.isnan(years_sepapr_19982021)], years_sepapr_19982021[~np.isnan(years_sepapr_19982021)])
...
for i, item in enumerate(np.arange(1997, 2021)):
    yeartemp_mardec = seaice_gerlache[(time_date_years == item) & ((time_date_months == 3) | (time_date_months == 4) |
    ...
    yeartemp_janmar = seaice_gerlache[(time_date_years == item+1) & ((time_date_months == 1) |
...
poly_ROI_NW = Polygon(list(south_verts), facecolor=[0,0,0,0.3], edgecolor='k', linewidth=1.5, linestyle=':',
                   zorder=2, transform=ccrs.PlateCarree())
plt.gca().add_patch(poly_ROI_NW)
plt.tight_layout()
#%%
