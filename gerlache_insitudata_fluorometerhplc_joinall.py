...
furg_df = pd.read_csv('chl_hplc_ready_withdatasets.csv', sep=';')
#furg_df.drop(furg_df[furg_df['subdataset'] != 'FURG'].index, inplace=True)
valente_df = pd.read_csv('valente2019_alldata_raw.csv', sep=',')
valente_df.drop(valente_df[valente_df['chla_hplc'] >0].index, inplace=True)
valente_df.drop(valente_df[valente_df['lat'] >-50].index, inplace=True)
valente_df.drop(valente_df[valente_df['lon'] <-70].index, inplace=True)
valente_df.drop(valente_df[valente_df['lon'] >-45].index, inplace=True)
# Join both matrices
lat_furg = furg_df['lat'].values
lon_furg = furg_df['lon'].values
chl_furg = furg_df['Chl'].values
date_furg = furg_df['date'].values
lat_val = valente_df['lat'].values
lon_val = valente_df['lon'].values
chl_val = valente_df['chla_fluor'].values
date_val = valente_df['time'].values
datetime_val = np.empty(len(date_val), dtype=object)
for i in range(0, len(datetime_val)):
    datetime_val[i] = datetime.datetime(year=int(date_val[i][:4]),
                                           month=int(date_val[i][5:7]),
                                           day=int(date_val[i][8:10]))
datetime_furg = np.empty(len(date_furg), dtype=object)
for i in range(0, len(datetime_furg)):
    datetime_furg[i] = datetime.datetime(year=int(date_furg[i][6:10]),
                                           month=int(date_furg[i][3:5]),
                                           day=int(date_furg[i][:2]))
lat_insitu = np.hstack((lat_furg, lat_val))
lon_insitu = np.hstack((lon_furg, lon_val))
chl_insitu = np.hstack((chl_furg, chl_val))
datetime_insitu = np.hstack((datetime_furg, datetime_val))
gerlache_verts = [(-68, -66),
                        (-64.5, -64.6),
                        (-64, -65),
             (-61.5, -64),
             (-61, -63.8),
             ...
             (-67.25, -66.8)]
#%%
# Find which points are located within the gerlache Area
points = np.vstack((lon_insitu, lat_insitu)).T
p = Path(gerlache_verts) # make a polygon
inside2 = p.contains_points(points) 
# Keep points only from within the region
chl_insitu_gerlache = chl_insitu[inside2]
datetime_insitu_gerlache = datetime_insitu[inside2]
# Remove values above 15
datetime_insitu_gerlache = datetime_insitu_gerlache[chl_insitu_gerlache<15]
chl_insitu_gerlache = chl_insitu_gerlache[chl_insitu_gerlache<15]
# Average for each Sep-April
time_date_years = np.empty_like(datetime_insitu_gerlache)
time_date_months = np.empty_like(datetime_insitu_gerlache)
for i in range(0, len(datetime_insitu_gerlache)):
    time_date_years[i] = datetime_insitu_gerlache[i].year
    time_date_months[i] = datetime_insitu_gerlache[i].month
# Count number of data per month
# SEPTEMBER
for i in np.arange(1997, 2019):
    sep_count = np.size(chl_insitu_gerlache[(time_date_years == i) & (time_date_months == 9)])
    if i == 1997:
...
# OCTOBER
for i in np.arange(1997, 2019):
    oct_count = np.size(chl_insitu_gerlache[(time_date_years == i) & (time_date_months == 10)])
    if i == 1997:
...
# NOVEMBER
for i in np.arange(1997, 2019):
    nov_count = np.size(chl_insitu_gerlache[(time_date_years == i) & (time_date_months == 11)])
    if i == 1997:
...
# DECEMBER
for i in np.arange(1997, 2019):
    dec_count = np.size(chl_insitu_gerlache[(time_date_years == i) & (time_date_months == 12)])
    if i == 1997:
...
# JANUARY
for i in np.arange(1997, 2019):
    jan_count = np.size(chl_insitu_gerlache[(time_date_years == i) & (time_date_months == 1)])
    if i == 1997:
...
# FEBRUARY
for i in np.arange(1997, 2019):
    feb_count = np.size(chl_insitu_gerlache[(time_date_years == i) & (time_date_months == 2)])
    if i == 1997:
...
# MARCH
for i in np.arange(1997, 2019):
    mar_count = np.size(chl_insitu_gerlache[(time_date_years == i) & (time_date_months == 3)])
    if i == 1997:
...
## Just Jan-Feb Means
for i in np.arange(1997, 2019):
    yeartemp_jan = chl_insitu_gerlache[(time_date_years == i) & (time_date_months == 1)]
    yeartemp_feb = chl_insitu_gerlache[(time_date_years == i) & (time_date_months == 2)]
#    yeartemp_mar = chl_insitu_gerlache[(time_date_years == i) & (time_date_months == 3)]
...
plt.text(1998, 20, f"R={r:.2f}", fontsize=14)
plt.text(1998, 17.5, f"p-val={pval:.2f}", fontsize=14)
plt.xlabel('Years', fontsize=14)
plt.ylabel('Chla', fontsize=14)
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\')
graphs_dir = 'gerlache_insituchlhplcfluo_janfebmean_onlyyearsabove10.png'
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\')
graphs_dir = 'gerlache_insituchlhplcfluo_janmarmean_onlyyearsabove10.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
#%%
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\')
graphs_dir = 'gerlache_insituchlhplcfluo_janfeb_counts_without2016bloom.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\')
graphs_dir = 'gerlache_insituchlhplcfluo_janmar_counts.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
#%% Plot Jan-Feb
(_, caps, _) = plt.errorbar(np.arange(1997, 2019), chlinsitu_janfeb19812021, yerr=chlinsitustd_janfeb19982021, capsize=3, elinewidth=1, linestyle='None', marker='o', c='k')
[slope,intercept,r,pval,_] = stats.linregress(np.arange(1997, 2019)[~np.isnan(chlinsitu_janfeb19812021)], chlinsitu_janfeb19812021[~np.isnan(chlinsitu_janfeb19812021)])
plt.plot(np.arange(1997,2019), np.arange(1997,2019)*slope+intercept, linestyle='--', c='b', alpha=0.5)
plt.text(1997, 10, f"R={r:.2f}", fontsize=14)
plt.text(1997, 9, f"p-val={pval:.2f}", fontsize=14)
plt.xlabel('Years', fontsize=14)
plt.ylabel('Chla', fontsize=14)
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\')
graphs_dir = 'gerlache_insituchlhplcfluo_janfebmean_without2016bloom_onlyyearsabove10.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
#%%
(_, caps, _) = plt.errorbar(np.arange(1997, 2019), chlinsitu_janmar19812021, yerr=chlinsitustd_janmar19982021, capsize=3, elinewidth=1, linestyle='None', marker='o', c='k')
...
plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\')
graphs_dir = 'gerlache_insituchlhplcfluo_janmarmean_without2016bloom_onlyyearsabove10.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()