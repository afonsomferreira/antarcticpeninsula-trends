...
par_10km = np.empty((len(lat_10km), len(lon_10km), len(time_date)))*np.nan
for i in tqdm(range(0, len(lat_10km))):
    for j in range(0, len(lon_10km)):
        if (i == len(lat_10km)-1) & (j != len(lon_10km)-1):
            ...
            lat_indices = (lat < lat_10km[i]) & (lat > lat_10km[i+1])
            lon_indices = (lon > lon_10km[j]) & (lon < lon_10km[j+1])
        par_10km[i, j, :] = np.nanmean(par[np.ix_(lat_indices, lon_indices)], (0,1))
#par_10km_clim = np.nanmean(par_10km, 2)
# Save datasets with lower resolution
np.savez_compressed('par_19972022_10km', par=par_10km,time_date=time_date,
