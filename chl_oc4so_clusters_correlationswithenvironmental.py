...
for i in np.arange(1998, 2023):
    ...
    if i == 1998:
        ges_mar_chl = np.nanmean(yeartemp_mar)
    else:
        ges_mar_chl = np.hstack((ges_mar_chl, np.nanmean(yeartemp_mar)))
## SST
...
for i in np.arange(1998, 2023):
    ...
    if i == 1998:
        ges_mar_sst = np.nanmean(yeartemp_mar)
    else:
        ges_mar_sst = np.hstack((ges_mar_sst, np.nanmean(yeartemp_mar)))
## Sea Ice
...
for i in np.arange(1998, 2023):
    yeartemp_mar = ges_cluster[(time_date_years_sst == i) & (time_date_months_sst == 3)]
    if i == 1998:
        ges_mar_seaice = np.nanmean(yeartemp_mar)
    else:
        ges_mar_seaice = np.hstack((ges_mar_seaice, np.nanmean(yeartemp_mar)))
## PAR
...
for i in np.arange(1998, 2023):
    ...
    if i == 1998:
        ges_mar_par = np.nanmean(yeartemp_mar)
    else:
        ges_mar_par = np.hstack((ges_mar_par, np.nanmean(yeartemp_mar)))
## Winds
...
for i in np.arange(1998, 2023):
    ...
    if i == 1998:
        ges_mar_winds = np.nanmean(yeartemp_mar)
    else:
        ges_mar_winds = np.hstack((ges_mar_winds, np.nanmean(yeartemp_mar)))
## Calculate correlations between chl and others
stats.spearmanr(ges_mar_chl, ges_mar_sst, nan_policy='omit')
stats.spearmanr(ges_mar_chl, ges_mar_seaice, nan_policy='omit')
stats.spearmanr(ges_mar_chl, ges_mar_par, nan_policy='omit')
stats.spearmanr(ges_mar_chl, ges_mar_winds, nan_policy='omit')
#%% BRS SEP, MAR, APR, SEP-APR
## Chl-a
...
for i in np.arange(1998, 2023):
    ...
    if i == 1998:
        brs_sep_chl = np.nanmean(yeartemp_sep)
        brs_mar_chl = np.nanmean(yeartemp_mar)
        brs_apr_chl = np.nanmean(yeartemp_apr)
    else:
        brs_sep_chl = np.hstack((brs_sep_chl, np.nanmean(yeartemp_sep)))
        brs_mar_chl = np.hstack((brs_mar_chl, np.nanmean(yeartemp_mar)))
        brs_apr_chl = np.hstack((brs_apr_chl, np.nanmean(yeartemp_apr)))
## SST
...
for i in np.arange(1998, 2023):
    ...
    if i == 1998:
        brs_sep_sst = np.nanmean(yeartemp_sep)
        brs_mar_sst = np.nanmean(yeartemp_mar)
        brs_apr_sst = np.nanmean(yeartemp_apr)
    else:
        brs_sep_sst = np.hstack((brs_sep_sst, np.nanmean(yeartemp_sep)))
        brs_mar_sst = np.hstack((brs_mar_sst, np.nanmean(yeartemp_mar)))
        brs_apr_sst = np.hstack((brs_apr_sst, np.nanmean(yeartemp_apr)))
## Sea Ice
...
for i in np.arange(1998, 2023):
    ...
    yeartemp_apr = brs_cluster[(time_date_years_sst == i) & (time_date_months_sst == 4)]
    if i == 1998:
        brs_sep_seaice = np.nanmean(yeartemp_sep)
        brs_mar_seaice = np.nanmean(yeartemp_mar)
        brs_apr_seaice = np.nanmean(yeartemp_apr)
    else:
        brs_sep_seaice = np.hstack((brs_sep_seaice, np.nanmean(yeartemp_sep)))
        brs_mar_seaice = np.hstack((brs_mar_seaice, np.nanmean(yeartemp_mar)))
        brs_apr_seaice = np.hstack((brs_apr_seaice, np.nanmean(yeartemp_apr)))
## PAR
...
for i in np.arange(1998, 2023):
    ...
    if i == 1998:
        brs_sep_par = np.nanmean(yeartemp_sep)
        brs_mar_par = np.nanmean(yeartemp_mar)
        brs_apr_par = np.nanmean(yeartemp_apr)
    else:
        brs_sep_par = np.hstack((brs_sep_par, np.nanmean(yeartemp_sep)))
        brs_mar_par = np.hstack((brs_mar_par, np.nanmean(yeartemp_mar)))
        brs_apr_par = np.hstack((brs_apr_par, np.nanmean(yeartemp_apr)))
## Winds
...
for i in np.arange(1998, 2023):
    ...
    if i == 1998:
        brs_sep_winds = np.nanmean(yeartemp_sep)
        brs_mar_winds = np.nanmean(yeartemp_mar)
        brs_apr_winds = np.nanmean(yeartemp_apr)
    else:
        brs_sep_winds = np.hstack((brs_sep_winds, np.nanmean(yeartemp_sep)))
        brs_mar_winds = np.hstack((brs_mar_winds, np.nanmean(yeartemp_mar)))
        brs_apr_winds = np.hstack((brs_apr_winds, np.nanmean(yeartemp_apr)))
## Calculate correlations between chl and others
# September
stats.spearmanr(brs_sep_chl, brs_sep_sst, nan_policy='omit')
stats.spearmanr(brs_sep_chl, brs_sep_seaice, nan_policy='omit')
stats.spearmanr(brs_sep_chl, brs_sep_par, nan_policy='omit')
stats.spearmanr(brs_sep_chl, brs_sep_winds, nan_policy='omit') # ***
# March
stats.spearmanr(brs_mar_chl, brs_mar_sst, nan_policy='omit')
stats.spearmanr(brs_mar_chl, brs_mar_seaice, nan_policy='omit')
stats.spearmanr(brs_mar_chl, brs_mar_par, nan_policy='omit')
stats.spearmanr(brs_mar_chl, brs_mar_winds, nan_policy='omit')
# April
stats.spearmanr(brs_apr_chl, brs_apr_sst, nan_policy='omit')
stats.spearmanr(brs_apr_chl, brs_apr_seaice, nan_policy='omit')
stats.spearmanr(brs_apr_chl, brs_apr_par, nan_policy='omit')
stats.spearmanr(brs_apr_chl, brs_apr_winds, nan_policy='omit')
#%%
...
stats.linregress(np.arange(1998, 2023)[~np.isnan(weds_sep)], weds_sep[~np.isnan(weds_sep)]) # *
stats.linregress(np.arange(1998, 2023)[~np.isnan(weds_oct)], weds_oct[~np.isnan(weds_oct)])
stats.linregress(np.arange(1998, 2023)[~np.isnan(weds_nov)], weds_nov[~np.isnan(weds_nov)])
stats.linregress(np.arange(1998, 2023)[~np.isnan(weds_dec)], weds_dec[~np.isnan(weds_dec)])
stats.linregress(np.arange(1998, 2023)[~np.isnan(weds_jan)], weds_jan[~np.isnan(weds_jan)])
stats.linregress(np.arange(1998, 2023)[~np.isnan(weds_feb)], weds_feb[~np.isnan(weds_feb)])
stats.linregress(np.arange(1998, 2023)[~np.isnan(weds_mar)], weds_mar[~np.isnan(weds_mar)]) #*
#stats.linregress(np.arange(1998, 2023)[~np.isnan(weds_apr)], weds_apr[~np.isnan(weds_apr)]) # not enough data
stats.linregress(np.arange(1998, 2023)[~np.isnan(weds_sepapr)], weds_sepapr[~np.isnan(weds_sepapr)])
stats.linregress(np.arange(1998, 2023)[~np.isnan(weds_decfeb)], weds_decfeb[~np.isnan(weds_decfeb)])
...
stats.linregress(np.arange(1998, 2023)[~np.isnan(ges_sep)], ges_sep[~np.isnan(ges_sep)])
stats.linregress(np.arange(1998, 2023)[~np.isnan(ges_oct)], ges_oct[~np.isnan(ges_oct)])
stats.linregress(np.arange(1998, 2023)[~np.isnan(ges_nov)], ges_nov[~np.isnan(ges_nov)])
stats.linregress(np.arange(1998, 2023)[~np.isnan(ges_dec)], ges_dec[~np.isnan(ges_dec)]) # *
stats.linregress(np.arange(1998, 2023)[~np.isnan(ges_jan)], ges_jan[~np.isnan(ges_jan)])
stats.linregress(np.arange(1998, 2023)[~np.isnan(ges_feb)], ges_feb[~np.isnan(ges_feb)])
stats.linregress(np.arange(1998, 2023)[~np.isnan(ges_mar)], ges_mar[~np.isnan(ges_mar)]) # ***
#stats.linregress(np.arange(1998, 2023)[~np.isnan(ges_apr)], ges_apr[~np.isnan(ges_apr)]) # not enough data
stats.linregress(np.arange(1998, 2023)[~np.isnan(ges_sepapr)], ges_sepapr[~np.isnan(ges_sepapr)])
stats.linregress(np.arange(1998, 2023)[~np.isnan(ges_decfeb)], ges_decfeb[~np.isnan(ges_decfeb)])
