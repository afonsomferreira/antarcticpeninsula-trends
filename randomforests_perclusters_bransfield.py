...
#%% Create dataframe for bransfield
## CHL
bransfield_cluster = chl[clusters == 4,:]
bransfield_cluster = np.nanmean(bransfield_cluster,0)
bransfield_cluster_df_chl = pd.Series(data=bransfield_cluster, index=time_date_chl)
bransfield_cluster_df_monthly_chl = bransfield_cluster_df_chl.resample('M').mean()
bransfield_monthly_19972021_chl = bransfield_cluster_df_monthly_chl.values[:-8]
bransfield_monthly_19972021_index = bransfield_cluster_df_monthly_chl.index[:-8]
## SST
bransfield_cluster = sst[clusters == 4,:]
bransfield_cluster = np.nanmean(bransfield_cluster,0)
bransfield_cluster_df_sst = pd.Series(data=bransfield_cluster, index=time_date_sst)
bransfield_cluster_df_monthly_sst = bransfield_cluster_df_sst.resample('M').mean()
bransfield_monthly_19972021_sst = bransfield_cluster_df_monthly_sst.values[8:-8]
## Sea Ice
bransfield_cluster = seaice[clusters == 4,:]
bransfield_cluster = np.nanmean(bransfield_cluster,0)
bransfield_cluster_df_seaice = pd.Series(data=bransfield_cluster, index=time_date_seaice)
bransfield_cluster_df_monthly_seaice = bransfield_cluster_df_seaice.resample('M').mean()
bransfield_monthly_19972021_seaice = bransfield_cluster_df_monthly_seaice.values[8:-8]
## PAR
bransfield_cluster = par[clusters == 4,:]
bransfield_cluster = np.nanmean(bransfield_cluster,0)
bransfield_cluster_df_par = pd.Series(data=bransfield_cluster, index=time_date_par)
bransfield_cluster_df_monthly_par = bransfield_cluster_df_par.resample('M').mean()
bransfield_monthly_19972021_par = bransfield_cluster_df_monthly_par.values[:-17]
## Join all in the same dataframe
dataframe_bransfield = pd.DataFrame(data=bransfield_monthly_19972021_chl, index= bransfield_monthly_19972021_index, columns=['Chla'])
dataframe_bransfield['SST'] = bransfield_monthly_19972021_sst
#dataframe_bransfield['SEAICE'] = bransfield_monthly_19972021_seaice
dataframe_bransfield['PAR'] = bransfield_monthly_19972021_par
dataframe_bransfield['MEI'] = mei_monthly_19972021
dataframe_bransfield['SAM'] = sam_monthly_19972021
#%%
...
#plt.tight_layout()
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\randomforests\\bransfield\\chla_monthlyvariation\\')
...
os.chdir('C:\\Users\\afons\\Documents\\artigos\\antarcticpeninsula-trends-2021\\analysis\\randomforests\\bransfield\\chla_monthlyvariation')
currentmodel_permut = pd.read_csv('permutation_bransfield_v1.csv', sep=";")
# Load sardine data
...
graphs_dir = 'bransfield_randomforestresults_V1.png'
plt.savefig(graphs_dir, format='png', bbox_inches='tight', dpi=300)
plt.close()
...
plt.close()
#%% Testes
plt.scatter(bransfield_monthly_19972021_chl, bransfield_monthly_19972021_sst)
stats.linregress(bransfield_monthly_19972021_chl[~np.isnan(bransfield_monthly_19972021_chl)], bransfield_monthly_19972021_sst[~np.isnan(bransfield_monthly_19972021_chl)])
