...
for i in np.arange(1998, 2022):
    yeartemp_sep = sst[:,:, (time_date_years == i-1) & (time_date_months == 9)]
    yeartemp_oct = sst[:,:, (time_date_years == i-1) & (time_date_months == 10)]
    ...
    yeartemp_mar = sst[:,:, (time_date_years == i) & (time_date_months == 3)]
    yeartemp_apr = sst[:,:, (time_date_years == i) & (time_date_months == 4)]
...
xx = xx[~np.isnan(sst_summertrend19992020_significant)]
yy = yy[~np.isnan(sst_summertrend19992020_significant)]
plt.figure()
...
map.contourf(
    lon, lat, sst_summertrend19992020_significant,
    transform=ccrs.PlateCarree(),
    colors='none',
    hatches=[3*'.',3*'.'],
)
#map.contour(lon, lat, sst_summertrend19992020_significant, levels=[0, 0.01], colors='black', zorder=1, transform=ccrs.PlateCarree())
...
plt.tight_layout()
graphs_dir = 'C:\\Users\\afons\\OneDrive - Universidade de Lisboa\\Documents\\artigos\\antarctic-peninsula-trends-2021\\analysis\\sst_trendmap_SEPAPR.png'
...
#cbar.ax.set_yticklabels(['0.1', '0.5', '1', '3', '10'], fontsize=14)
cbar.set_label('SST (degC)', fontsize=14)
#map.contour(lon, lat, sst_summertrend19992020_significant, levels = [0.9, 1.1], colors='k',
#            transform=ccrs.PlateCarree())
#plt.contour(X, Y, Z, levels=[0.5, 0.75], colors=['black','cyan'])
#cbar.set_label('Valid Pixels (%)', fontsize=14)
map.coastlines(resolution='10m', color='black', linewidth=1)
map.add_feature(cartopy.feature.NaturalEarthFeature('physical', 'land', '50m',
                                        ...
                                        facecolor=cartopy.feature.COLORS['land']))
plt.tight_layout()
graphs_dir = 'C:\\Users\\afons\\OneDrive - Universidade de Lisboa\\Documents\\artigos\\antarctic-peninsula-trends-2021\\analysis\\sst_trendmap_SEPAPR_onlysignificant.png'
plt.savefig(graphs_dir,format = 'png', bbox_inches = 'tight', dpi = 300)
plt.close()